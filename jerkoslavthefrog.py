...
def obtain_offers_via_JustWatch(country_query = 'US', movieTitle = 'Licence to Kill'):
    ...
    # first title returned is most relevant usually >> obtain IMDB id from this title
    ...
    offers              = 'none' # default
    if 'offers' in list(JWquery.keys()): # JSON to pandas
...
try: # for local running (this folder/file is not tracked by git)
...
default_director    = ['Christopher Nolan','Quentin Tarantino']
...
st.sidebar.write("Ratko mjauuu")

page = st.sidebar.radio('Browse:', ['Movies','Movie People'])
#########################################################################################################################
# page specific section: different filtering of movies based on selected browsing type ##################################
if page == 'Movies': # browse/select/filter by movie criteria
...
elif page == 'Movie People': # browse/select/filter by people in movies
    ...
    if len(person_types_select) > 1: 
        # if more than one person type is selected, e.g. actress & director, then add the category as a suffix for those people that fall in more than one category.
    ...
    movies_by_personCategory_stacked = people_selected_category[people_selected_category[name_col].isin(people_selection)].explode('tconst')
    movies_persons = movies_by_personCategory_stacked.merge(movies, on = 'tconst', how = 'left').sort_values(by = 'Year')
    ...
    movies_subset  = movies_subset.sort_values(by = ['Rating','Title'], ascending = False).reset_index()
    # create line graph with movie scores per person over time
    fig = px.line(movies_subset.sort_values(by = ['Year', name_col]), x="Year", y="Rating", markers = True, \
                  title='Movie Scores over Time for Each Person Selected', \
                    color = name_col, hover_name = 'Title')
    st.plotly_chart(fig, use_container_width=True) # set size better
#################################################################################################################################################

######################### SELECTED/FILTERED SUBSET MOVIES DETAILS (applicable to both pages) ####################################################
st.markdown("""---""")
st.subheader(f'{movies_subset.shape[0]} movies meet the specified criteria')
color_by = 'Genres' if page == 'Movies' else name_col
show_columns_movies_subset = show_columns_movie_page if page == 'Movies' else show_columns_persons_page
suffix_table_expander = 'that match the selection criteria' if page == 'Movies' else 'for selected people'
# 3D interactive visualization
fig3d = px.scatter_3d(movies_subset, y='Runtime', z='Rating', x='Votes', color=color_by, hover_name = 'Title')
fig3d.update_layout(
title={
...
see_selected_movies = st.expander('👉 Click here to see movies ' + suffix_table_expander)
with see_selected_movies:
    st.dataframe(movies_subset[show_columns_movies_subset].style.format({'Year': '{:.0f}', 'Rating': '{:.1f}',\
